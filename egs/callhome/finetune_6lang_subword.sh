...
--train-subset train --valid-subset dev_ar,dev_en,dev_ge,dev_ja,dev_ma,dev_sp --no-epoch-checkpoints --best-checkpoint-metric uer \
--num-workers 4 --max-update 160000 --sentence-avg --task audio_pretraining --arch wav2vec_ctc --w2v-path $W2V_PATH \
...
--feature-grad-mult 0.0 --freeze-finetune-updates 10000 --validate-after-updates 10000  --validate-interval 10 \
