...
--train-subset train --valid-subset valid --no-epoch-checkpoints --best-checkpoint-metric loss \
--num-workers 2 --max-update 80000 \
--task audio_cif_pretraining --arch wav2vec_cif --w2v-path $W2V_PATH --labels $label_type \
--assigner-conv-layers '[(512,5,1)] * 1 + [(512,3,1)] * 1' \
...
--attention-dropout 0.0 --max-tokens 900000 --seed 2337 --ddp-backend no_c10d --update-freq 12 \
--log-interval 20 --log-format simple --save-interval 10
