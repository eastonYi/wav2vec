...
gpu=$1
SAVE_DIR=exp/finetune_ctc_ce_en_subword
...
--train-subset train --valid-subset dev --no-epoch-checkpoints --best-checkpoint-metric acc \
--num-workers 2 --max-update 80000 \
--task asr --arch wav2vec_ctc_lm --w2v-path $W2V_PATH --labels $label_type \
...
--final-dropout 0.0 --dropout 0.0 --activation-dropout 0.1 --criterion ctc_ce --zero-infinity \
--attention-dropout 0.0 --max-tokens 1000000 --seed 2337 --ddp-backend no_c10d --update-freq 6 \
--log-interval 1 --log-format simple --save-interval 20
